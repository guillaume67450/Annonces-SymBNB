...
class Booking
{
     ...
     * @Assert\GreaterThan("today", message="La date d'arrivée doit être ultérieure à la date du jour")
     */
    private $startDate;
     ...
     * @Assert\GreaterThan(propertyPath="startDate", message="La date de départ doit être ultérieure la date d'arrivée !")
     */
    private $endDate;
    ...
    public function isBookableDate() {
        // 1) il faut connaître les dates qui sont impossibles (déjà réservé)
        $notAvalaibleDays = $this->ad->getNotAvailableDays();

        // 2) Il faut comparer les dates choisies avec les dates possibles
        ...
        $notAvalaible   = array_map($formatDay, $notAvalaibleDays);
    ...
    public function getDays() {
        ...
        //on transforme le résultat contenant des timestamp en tableau de datetime
